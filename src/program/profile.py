...
rules = {
    ...
    "G10": Rule("C-G10", "Inline Assembly", Severities.FATAL, "Inline assembly must never be used", rules.C_G10.checker, False),
    ...
    "F7": Rule("C-F7", "Structures as parameters", Severities.MAJOR, "Structures must be transmitted as arguments using a pointer, not by copy", rules.C_F7.checker, False),
    ...
    "L1": Rule("C-L1", "Code line content", Severities.MAJOR, "A line must correspond to only one statement", rules.C_L1.checker, False),
    ...
    "L5": Rule("C-L5", "Variable declarations", Severities.MAJOR, "Variables must be declared at the beginning of the scope of the function, one per line", rules.C_L5.checker, False),
    "L6": Rule("C-L6", "Line jumps", Severities.MINOR, "No other line breaks must be present in the scope of a function except after variable declarations", rules.C_L6.checker, False),

    "V1": Rule("C-V1", "Naming identifiers", Severities.MINOR, "Identifiers should be correctly named", rules.C_V1.checker, False),
    "V3": Rule("C-V3", "Pointers", Severities.MINOR, "The asterisk (*) must be attached to the associated variable, with no spaces in between, and preceded by a space", rules.C_V3.checker, False),
    ...
    "C2": Rule("C-C3", "Ternary operators", Severities.MAJOR, "You must never use nested or chained ternary operators", rules.C_C2.checker, False),
    ...
    "FN": Rule("C-FN", "Banned function", Severities.MAJOR, "This syscall is usually banned in projects", rules.C_FN.checker, True),
}
